...
if __name__ == '__main__':
    ...
    pretrained = 'coco_word_decoder_80_1e-05.pkl'
    lstm.load_state_dict(torch.load(pretrained))  ###
    # lstm.eval()  ###
    # lstm.train()  ###
    print(pretrained, ' model loaded, resume training')
    criterion = nn.NLLLoss(reduction='sum')
    # learning rate
    learning_rates = [0.000005, 0.0000025, 0.000001, 0.0000005]
    # optimizer
    epoch_0 = 81  # decending lr
    start = time.time()
    for learning_rate in learning_rates:
        ...
        n_epoch = 10
        total_loss = 0
        # save_every = 5  # save once epochs are finished
        print("learning rate: ", str(learning_rate))  ######

        print("epochs\tloss\t\ttotol time (min)")
        for epoch in range(epoch_0, n_epoch+epoch_0):
 ...
 #               break  #############
  ...
  #              break  #############
   ...
   #             break  #############
        ...
        out_file_name = 'coco_word_decoder_' + str(epoch) + '_' + str(learning_rate) + '.pkl'

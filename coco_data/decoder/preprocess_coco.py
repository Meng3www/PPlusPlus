...
def getCocoData():
    ...
    cnn.load_state_dict(torch.load('coco-encoder-5-3000.pkl', map_location=device))
    ...
    transform = transforms.Compose([
        ...
        transforms.Normalize((0.485, 0.456, 0.406),
                             (0.229, 0.224, 0.225))])
    json_data = json.loads(open('captions_train2014.json', 'r').read())
    with open("vocab_coco_4533.pkl", 'rb') as f:
        vocab = pickle.load(f)
    # slice json_data['annotations'] to continue get test_data from 56k, 70k, 84k
    # print("json_data length: ", str(len(json_data['annotations'])))  # 414113
    anno_data = json_data['annotations'][40000:]  ############## 
    print("after slicing: ", str(len(anno_data)))
    #################################

    for each_img in anno_data:
...
if __name__ == '__main__':
    ...
    #     break
    #########################
    # 10k pairs for training
    print("index\ttime\tfile_name")
    batch_temp = []  # batch container
    counter = 0
    start = time.time()
    file_index = 40
    for feature, captions, _, _ in getCocoData():
        batch_temp.append((feature, captions))
        counter += 1
        if counter % 12426 == 0:
            filename = 'coco_train_' + str(file_index) + ".pkl"
            file_index += 1
            with open(filename, 'wb') as fp:
                torch.save(batch_temp, fp)
            batch_temp = []  # reset the container
            print(str(counter), "\t", str((time.time() - start)/60), "\t", filename)
            break  ##########  break every 10k is done
        elif counter % 1000 == 0:  # time tracking

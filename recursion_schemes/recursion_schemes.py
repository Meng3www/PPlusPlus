...
from utils.image_and_text_utils import max_sentence_length, index_to_char, vectorize_caption
"""
...
def likelihood(self,depth=0,which_image=0,speaker_rationality=1.0,speaker=0,
			listener_rationality=1.0,img_prior=np.log(np.asarray([0.5,0.5])),start_from=""):
...
def ana_greedy(rsa,initial_world_prior,speaker_rationality,speaker, target, pass_prior=True,
	...
	depth:
		the number of levels of RSA: depth 0 uses listeral speaker to unroll,
		depth n uses speaker n to unroll, and listener n to update at each step
	start_from:
	...
	context_sentence = ['<start>']+start_from
	...
	for timestep in tqdm(range(len(start_from)+1, max_sentence_length)):
		...
		# print("S:",s)	 # ndarray (4987,) float

		segment = np.argmax(s)  ######### index
		# print("s",rsa.idx2seg[segment])
		prob = np.max(s)  # neg float
		probs.append(prob)  # list of neg float

		if pass_prior:

			l = rsa.listener(state=state,utterance=segment,depth=depth)
			state.world_priors[state.timestep]=l
		state.context_sentence += [rsa.idx2seg[segment]]  # ['<start>', 'shopping']
...
# But within the n-th order ethno-metapragmatic perspective, this creative indexical effect
# is the motivated realization, or performable execution, of an already constituted framework of semiotic value.
def ana_beam(rsa,initial_world_prior,speaker_rationality, target,speaker, pass_prior=True,listener_rationality=1.0,
	...
	decay_rate: a multiplier that makes later decisions in the unrolling matter less: 0.0 does no decay.
		negative decay makes start matter more
	"""
